...
const ImageCard = ({ src, name, mCap, url, isTopClipped, isBottomClipped }) => {
  ...
  return (
    <div
      ...
      style={{
        overflow: "hidden",
        borderRadius: isTopClipped ? "0 0 48px 48px" : isBottomClipped ? "48px 48px 0 0" : "48px",
...
const AIUniverseGrid = ({ data }) => {
  ...
  return (
    ...
    >
      <h1
        className="absolute z-10 text-[32px] leading-[40px] sm:text-5xl font-bold text-center text-white transform -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2"
      ...
      {rows.map((index) => {
        ...
        return (
          ...
          >
            <Marquee loop={false} pauseOnHover direction={directions[index]} speed={50}>
              <div
